import { Component, ComponentRef, ViewChild, ViewContainerRef, output, signal, input } from '@angular/core';
...
@Component({
    ...
    selector: 'output-function-child',
})
...
@Component({
    ...
    template: `
        @if (!componentRef) {
            <my-button [buttonName]="'Render dynamically!'" (click)="onClick()" #button/>
        }
...
export class OutputFunctionComponent {
    ...
    @ViewChild('placeToRender', {read: ViewContainerRef}) placeToRenderRef!: ViewContainerRef;
    @ViewChild('button', {read: ViewContainerRef}) buttonRef!: ViewContainerRef;
    componentRef!: ComponentRef<any>;
    outputObservable!: Subscription;
    
    onClick() {
        this.componentRef = this.placeToRenderRef.createComponent(OutputFunctionChildComponent);
        ...
        this.outputObservable = outputToObservable<string>(this.componentRef.instance.outputValue) // Observable<string>
        .pipe(
            concatMap(name => {
